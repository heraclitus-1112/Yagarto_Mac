...
- 导入不改写源码内容；工程校验并发布成功后才删除原文件。读取使用不跟随链接的文件快照，删除前重新核对文件身份与字节；若文件变化或删除失败，会保留两份并明确警告，不会丢失源码。动态预处理、宏或条件汇编决定入口时会安全跳过，由用户通过兼容 `init` 流程显式配置。
- 同名工程不会被覆盖，而是自动使用 `-2`、`-3` 后缀。符号链接、硬链接、非普通文件、非 UTF-8、过大文件和已有工程内的源码会被拒绝。
...
左侧工程导航器在单文件和多文件工程中都会显示，并只列出 `yagarto.json` 的 `sources`。目录树由相对路径派生，不扫描或显示 `.yagarto` 构建目录。底部 `+` 菜单可以在工程内新建空白 `.s/.S`，或把一个或多个现有汇编文件复制进工程；复制不会移动原文件，同名项使用 `-2`、`-3` 后缀且不会覆盖。右键源码可重命名、复制相对路径、在 Finder 中显示或移到废纸篓。重命名只改变当前目录中的文件名；首版不支持拖拽排序、新建文件夹或跨目录移动。
